...
try {
  if (fs.existsSync(STATE_FILE)) {
    ...
    // If it was running before crash, pause it
...
const addLog = (log: Omit<EnrichmentLog, "timestamp">) => {
  state.logs.unshift({ ...log, timestamp: new Date().toISOString() });
  if (state.logs.length > 500) {
    state.logs.pop(); // keep last 500
  }
};
...
Respond ONLY with valid JSON in this exact structure:
{
  ...
  "reasoning": "brief explanation"
    ...
    if (config.autoRetry && state.retriesCount < 100) {
       // We won't endlessly retry, let's keep it simple: manual retry for failed
    }
  }
}
